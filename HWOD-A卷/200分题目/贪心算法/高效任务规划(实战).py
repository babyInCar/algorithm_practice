...
def calc_min_cost_time(matrix):
    ...
    for config_time, run_time in matrix:
        ...
        res = max(res, total_config_time + run_time)
    return res
...
def main():
    a = int(input())
    for i in range(a):
        task_tuple = [list(map(int, input().split())) for x in range(int(input()))]
        print(calc_min_cost_time(task_tuple))
